...
IMPORTANT: If NONE of the provided tables can answer the query, you MUST return exactly "none_of_these" (without quotes).

Return "none_of_these" in these cases:
- The query asks for MORE SPECIFIC categories than available (e.g., query asks for "GST sector-wise" data but tables only have "GST overall" data)
- The query EXPLICITLY mentions a time frequency (monthly, quarterly, annual, yearly, weekly, daily, etc.) that is not available in the sample data

DO NOT return "none_of_these" if:
- The query asks for LESS SPECIFIC categories than available (e.g., query asks for "GST overall" and tables have "GST sector-wise" - sector-wise can be aggregated)
- The sample rows show similar granularity data exists (e.g., sample shows services data when query asks for manufacturing, or sample shows one sector when query asks for another sector). The sampling may not have retrieved the exact match, but the entry likely exists in the table.
- For international trade or foreign investment queries: If the query mentions FDI or FDI for India, the table will contain mention of other countries. India will not be explicitly mentioned. Do NOT return "none_of_these" just because India is not explicitly mentioned - the data represents India's trade/investment with those countries.
- If the query only mentions a date range (e.g., \"May 2025 to October 2025\") without specifying frequency, then choose the table with the SMALLEST granularity available (e.g., prefer monthly over quarterly, quarterly over annual). Do NOT return "none_of_these" if the query only mentions a date range without specifying frequency.

Otherwise, return ONLY the table name, nothing else."""
            ...
            user_prompt_parts.append(f"Return \"none_of_these\" if the query requires MORE granular data than available ")
            user_prompt_parts.append(f"(e.g., sector-wise requested but only overall available). ")
            user_prompt_parts.append(f"More granular data is acceptable (e.g., sector-wise data if overall is asked). ")
            user_prompt_parts.append(f"If the sample rows show similar granularity data exists (e.g., USA in sample when UK is asked, ")
            user_prompt_parts.append(f"or one sector in sample when another sector is asked), do NOT return \"none_of_these\" - ")
            user_prompt_parts.append(f"the exact match may exist in the table even if not in the sample. ")
            user_prompt_parts.append(f"For international trade or foreign investment queries (e.g., \"FDI sector-wise\", \"FDI for India\"), ")
            user_prompt_parts.append(f"trade/investment with India is implied - do NOT return \"none_of_these\" just because India is not explicitly mentioned. ")
            user_prompt_parts.append(f"\nTIME FREQUENCY LOGIC: ")
            user_prompt_parts.append(f"If the unit_query mentions a time frequency (monthly, quarterly, annual, yearly, weekly, daily, etc.), ")
            user_prompt_parts.append(f"then choose a table with the SAME frequency. If no table with the matching frequency is available, return \"none_of_these\". ")
            ...
            user_prompt_parts.append(f"then choose the table with the SMALLEST granularity available (e.g., prefer monthly over quarterly, quarterly over annual). ")
            user_prompt_parts.append(f"\nReturn ONLY the table name (e.g., \"{matches[0]['table_name']}\"), with no additional text or explanation.")
